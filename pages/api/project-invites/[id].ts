...
async function handlePatch(req: NextApiRequest, res: NextApiResponse) {
    ...
    if (!id || typeof id !== 'string' || !status || !userId) {
        return res
            .status(400)
            .json({ error: 'Invite ID, status and userId are required' });
    }
    ...
    try {
        ...
        if (inviteData.receiver_id !== userId) {
            return res
                .status(401)
                .json({ error: 'Unauthorized user to change status' });
        }

         // Update the project invite
         const { error: updateInviteError } = await supabase
             .from('project_invites')
             .update({ status: status, isInviteSentRead: false })
             .eq('id', id);

         if (updateInviteError) {
             console.error('Error updating project invite:', updateInviteError);
            return res.status(500).json({ error: 'Failed to update project invite' });
         }
        ...
        if (status === 'accepted') {
            ...
            if (collaboratorError) {
                console.error(
                    'Error creating project collaborator:',
                    collaboratorError,
                );
                return res
                    .status(500)
                    .json({ error: 'Failed to create project collaborator' });
            }
            return res
                .status(200)
                .json({ message: 'Invite accepted, and collaborator created' });
