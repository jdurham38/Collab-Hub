...
export const applyToProject = async (
    ...
    try {
      ...
      if (response.status !== 201){
          throw new Error(`Failed to apply to project, status code: ${response.status}`);
      }
    } catch (error) {
      if (axios.isAxiosError(error)) {
          ...
          throw new Error('An unexpected error occurred while applying to the project.');
      }
    }
  };
    ...
    try {
      const response = await axios.get(
       ...
       if (response.status !== 200){
          throw new Error(`Failed to check application status, status code: ${response.status}`);
       }
      return { hasApplied: response.data.hasApplied};
        ...
        if (axios.isAxiosError(error)) {
            ...
            throw new Error('An unexpected error occurred while checking application status.');
        }
    }
  };