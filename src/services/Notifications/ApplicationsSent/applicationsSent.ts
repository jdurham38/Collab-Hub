  ...
  fetchApplicantProjectRequests: async (userId: string) => {
    try {
      ...
      if (error instanceof AxiosError) {
          ...
          throw new Error(
            error.response?.data?.error || 'Failed to fetch applicant project requests'
          );
      }else if (error instanceof Error) {
           console.error('Error fetching applicant project requests:', error);
           throw new Error(error.message || 'Failed to fetch applicant project requests');
      } else {
         console.error('Error fetching applicant project requests:', error);
