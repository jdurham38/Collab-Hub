    ...
    useEffect(() => {
        ...
        const fetchDirectMessages = async () => {
            ...
            try {
                ...
                if (supabaseError) {
                    console.error('supabase error fetching direct messages:', supabaseError);
                    setError(supabaseError.message);
                } else if (data) {
                    const uniqueUserIds = Array.from(new Set(data.map((msg) => msg.sender_id)));
                    ...
                    const messagesWithUser = data.map((message) => {
                         const user = userMap[message.sender_id] || { username: 'Unknown User', email: '' };
