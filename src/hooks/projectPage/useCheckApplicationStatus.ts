...
const useCheckApplicationStatus = (
  projectId: string,
  userId: string | null,
): UseCheckApplicationStatusResult => {
  ...
  useEffect(() => {
    const fetchApplicationStatus = async () => {
      ...
      setError(null);
      try {
        const response = await checkApplicationStatus(projectId, userId);
        const { hasApplied: initialHasApplied } =
          response as CheckApplicationStatusResponse;
